...
export const useCart = () => {
  ...
  const subtotal = items.reduce((total: number, item: CartItem) => total + item.totalPrice, 0);
  ...
  const applyCoupon = useCallback(async (code: string) => {
    try {
      ...
      if (Config.ENABLE_ANALYTICS && result.type.endsWith('fulfilled')) {
        await analytics().logEvent('coupon_applied', {
          coupon_code: code,
          cart_value: subtotal,
...
export default function CartNewRouteNotFound() {
  return null;
}
